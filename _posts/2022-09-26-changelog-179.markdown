...
* [#9176] Use the [`--keep-going` Cargo option](https://doc.rust-lang.org/nightly/cargo/reference/unstable.html#keep-going) when building and evaluating build scripts. This allows building and executing as much as possible even if some build scripts or proc-macros aren't working. Requires enabling of the `org.rust.cargo.evaluate.build.scripts` [experimental feature][experimental features]
